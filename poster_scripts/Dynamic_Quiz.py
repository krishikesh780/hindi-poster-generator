...
try:
    ...
    st.error("❌ Configuration file `data.json` not found.")
    st.stop()
except json.JSONDecodeError as e:
    st.error(f"❌ Failed to parse `data.json`: {e}")
    st.stop()
...
# --- STREAMLIT CONFIG (अब सही जगह पर) ---
...
def load_wallpapers(category_path):
    ...
    if not os.path.exists(category_path):
        st.warning(f"⚠️ Wallpaper folder not found: {category_path}")
        return images
...
if st.button(labels["generate_button"], use_container_width=True):
    ...
    try:
        ...
        st.error(f"❌ Failed to generate posters: {e}")
        st.stop()
    ...
    st.download_button(labels["download_zip"], data=zip_buffer.getvalue(),
                       file_name=f"{selected_topic}_Quiz_Posters.zip", mime="application/zip",
                       use_container_width=True)

    st.download_button(labels["download_pdf"], data=pdf_buffer.getvalue(),
